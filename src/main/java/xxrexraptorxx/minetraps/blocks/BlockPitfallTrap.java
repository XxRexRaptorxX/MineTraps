...
public class BlockPitfallTrap extends Block {
    ...
    protected final VoxelShape CUSTOM_COLLISION_AABB =
            Block.createCuboidShape(0.0625D, 0.0625D, 0.0625D, 15.9375D, 15.9375D, 15.9375D);

    public BlockPitfallTrap(AbstractBlock.Settings settings) {
        super(settings.mapColor(MapColor.OAK_TAN)
                .nonOpaque()
                .noCollision()
                .strength(1.0F, 1.0F)
    ...
    public void onEntityCollision(
        ...
        world.playSound(
                null, pos.getX(), pos.getY(), pos.getZ(), SoundEvents.ENTITY_ITEM_BREAK, SoundCategory.BLOCKS, 1.0F, 3);
        ...
        if (state.getBlock() == ModBlocks.PITFALL_TRAP && state.get(TYPE) == 0) {
            ...
            if (TrapHelper.getTypeList().contains(player.getStackInHand(hand).getItem())) {
                world.setBlockState(
                        ...
                        state.with(
                                TYPE,
                                TrapHelper.getStateFromBlock(Registries.ITEM
                                        .getId(player.getStackInHand(hand).getItem())
                                        .toString())),
                        2);
                world.playSound(
                        null,
